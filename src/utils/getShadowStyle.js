export default function getShadowStyle(phase, moonSize){
    ...
    function getShadowState(phaseProgres){
        ...
        else if(phaseProgres === 0.75) return ({...initialPhaseState, halfShadow: 1, shadowDir: 1})
        else if(phaseProgres < 0.25) return ({...initialPhaseState})
        else if(phaseProgres > 0.25 && phaseProgres < 0.5) return ({...initialPhaseState, progresState: 1})
        else if(phaseProgres >= 0.50 && phaseProgres < 0.75) return ({...initialPhaseState, progresState: 1, shadowDir: 1})
        else if(phaseProgres > 0.75) return ({...initialPhaseState, progresState: 0, shadowDir: 1})
    }
    ...
    function getBoxShadowParams(shadowDir, spreadSelector,{x, y, blur, spread} = boxShadowParams){
        if(!spreadSelector){
            spread+=x;
            x=0;
        }else{
        ...
        let mainShadow = `${shadowDir&&'-'}${x}px ${y}px ${blur}px ${spread}px ${shadowColor}`
    return {
            0: mainShadow,
            1: `0 0 0 400px ${shadowColor}, inset ${mainShadow}`
        }
    }
    function getSizeAndPosition({progresState, shadowDir, halfShadow, phaseProgres} = initialPhaseState){
        let boxShadow = getBoxShadowParams(shadowDir, progresState ,boxShadowParams)
        let background = {
            0: `${shadowColor}`,
            1: "transparent"
        }
        ...
        Number((phaseProgres > 0.5 ? 0.75 - phaseProgres : phaseProgres - 0.25)).toPrecision(5)
        :
        Number((phaseProgres > 0.5 ? phaseProgres - 0.75 : 0.25 - phaseProgres)).toPrecision(5)
        ;
        ...
        let shadowTranslateX = (newRadius - moonCenterToShadowEdge) +  getShadowDirection(shadowDir ,progresState) * shadowCenterOffset;
