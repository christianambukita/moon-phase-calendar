		...
		<div ref={calendarRef} className='calendar-container'>
			<div className='calendar'>
				...
				<div className='calendar-entries'>
					{mArray.map((date, index) => {
						...
						if (date) {
									...
									{size.width > 460 && (
										<div className='entry-details'>
											<span className='phase-name'>
												{getPhaseName(phase, PHASE_NAMES)}
											</span>
											<span className='phase-name'>Moon age: </span>
											<span className='phase-name'>
												{roundWrap(phase * 29.5, 1)} days (
												{roundWrap(phase * 100, 1)}%)
											</span>
										</div>
									)}
								</div>
							...
							return (
								<div
									className='calendar-entry empty'
									key={index}
									style={{ flexBasis: entryWidth, margin: `${gap}%` }}></div>
							);
						}
					})}
				</div>
				<div className='date-info'>
					<span className='phase-name'>
						{getPhaseName(refPhase, PHASE_NAMES)}
					</span>
					<span className='phase-name'>Moon age: </span>
					<span className='phase-name'>
